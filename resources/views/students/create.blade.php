...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4">
    <div>
        <h2 class="h3 mb-1 fw-bold text-primary">Add New Student</h2>
        <p class="text-muted mb-0">Daftarkan mahasiswa baru ke dalam sistem</p>
    </div>
    <div class="mt-3 mt-md-0">
        <a href="{{ route('students.index') }}" class="btn btn-light border shadow-sm rounded-pill px-4">
    ...
    <div class="col-md-8">
        <div class="card border-0 shadow-sm rounded-4 mb-5">
            <div class="card-header bg-transparent border-0 pt-4 pb-2 px-4">
                <h5 class="fw-bold mb-0 text-primary"><i class="fas fa-user-graduate me-2"></i> Student Information</h5>
            </div>
            <div class="card-body px-4 pb-4">
                <form action="{{ route('students.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="name" class="form-label">Full Name (From Model) <span class="text-danger">*</span></label>
                                <select class="form-select @error('name') is-invalid @enderror"
                                    id="name" name="name" required>
                                    <option value="">-- Pilih Nama dari AI Model --</option>
                                    @foreach($availableNames as $n)
                                        <option value="{{ $n }}" {{ old('name') == $n ? 'selected' : '' }}>{{ $n }}</option>
                                    @endforeach
                                </select>
                                @error('name')
                                <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="d-flex justify-content-end gap-2 mt-5 border-top pt-4">
                        <a href="{{ route('students.index') }}" class="btn btn-light border rounded-pill px-4">Cancel</a>
