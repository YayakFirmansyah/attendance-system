                        ...
                        @if ($todayAttendanceList->count() > 0)
                            @foreach ($todayAttendanceList as $attendance)
                                <div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-2">
                                    <div>
                                        <strong>{{ $attendance->student->name ?? 'Unknown Student' }}</strong><br>
                                        <small class="text-muted">
                                            Kelas: {{ $attendance->classModel->course->course_name ?? 'Unknown Class' }}
                                        </small>
                                    </div>
                                    <div class="text-end">
                                        <span
                                        ...
                                        <br>
                                        <small
                                            class="text-muted">{{ $attendance->check_in ? \Carbon\Carbon::parse($attendance->check_in)->format('H:i') : '--:--' }}</small>
                                    </div>
                                </div>
                            @endforeach
                        @else
                                ...
                                <div class="d-flex align-items-center mb-3">
                                    <div class="flex-shrink-0">
                                        <div class="bg-{{ $log->is_verified ? 'success' : 'warning' }} rounded-circle d-flex align-items-center justify-content-center"
                                            ...
                                            <i
                                                class="fas fa-{{ $log->is_verified ? 'check' : 'exclamation' }} text-white small"></i>
                                        </div>
                                    </div>
